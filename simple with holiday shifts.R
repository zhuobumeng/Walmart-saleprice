...
###############################################
feature=read.csv("E:/xf/UIUC/reg/features.csv")
train=read.csv("E:/xf/UIUC/reg/train.csv")
store=read.csv("E:/xf/UIUC/reg/stores.csv")
test=read.csv("E:/xf/UIUC/reg/test.csv")
...
# Remove "MarkDown", "CPI", unemployment -- many participants said they are not much useful
remove.var = NULL; 
for(k in 1:5) remove.var=c(remove.var, paste("MarkDown", k, sep=''))
mytrain = mytrain[, !names(mytrain) %in% remove.var]
mytrain = mytrain[, !names(mytrain) %in% c("CPI","Unemployment")]
mytest = mytest[, !names(mytest) %in% remove.var]
mytest = mytest[, !names(mytrain) %in% c("CPI","Unemployment")]
...
mytrain$logsales=log(mytrain$sales+4990)
...
for(k in 1:length(dept.names)){
  ...
  file.name = paste("fig_sales_dept_", dept.names[k], ".png", sep="")
  png(file.name)
  tmp=subset(mytrain, dept==dept.names[k])
  tmp$wk_index = as.numeric(difftime(tmp$date, day1, units="weeks"))
...
for(i in 1:num.depts){
  ...
  for(j in 1:num.stores){
    ...
    # cumulative weeks; set day 1 in training as the reference
    day1 = yday("2010-02-05")
    cum.wk.train = (mytrain$year[tmp.train.id] -2010)*365+(mytrain$year[tmp.train.id]>2012)+mytrain$yday[tmp.train.id]
    cum.wk.train = (cum.wk.train - day1)/7+1; 
    cum.wk.test = (mytest$year[tmp.test.id] -2010)*365+(mytest$year[tmp.test.id]>2012)+mytest$yday[tmp.test.id]
    cum.wk.test = (cum.wk.test - day1)/7 + 1; 
    ...
    hist.data = matrix(, length(tmp.test.id), 1)
    hist.data[,1] = tmp.train.sales[cum.wk.test-52];
    
    med.pred = apply(hist.data, 1, function(x) mean(x[!is.na(x)]))
    for(k in 1:length(tmp.test.id))
    {
      if(cum.wk.test[k]==152 && !is.na(tmp.train.sales[cum.wk.test[k]-53]))
      {
        med.pred[k]=tmp.train.sales[cum.wk.test[k]-52]*4/7+tmp.train.sales[cum.wk.test[k]-53]*3/7
      }
      else if(cum.wk.test[k]==165 &&!is.na(tmp.train.sales[cum.wk.test[k]-51]))
      {
        med.pred[k]=tmp.train.sales[cum.wk.test[k]-51]
      }
    }
    
    # If no prior year's data available, just predict it to be the median
    # of the other predicted values
    med.pred[is.na(med.pred)] = median(tmp.train.sales, na.rm=TRUE)
...
tmp.id = which(mytest$missing==1)
#for(i in 1:length(tmp.id)) mypred[tmp.id[i]]=median(mytrain$sales[which(mytrain$dept==mytest$dept[tmp.id[i]])])
for(i in 1:length(tmp.id)) mypred[tmp.id[i]]=0
##################################################################
# WRITE YOUR CODE HERE
...
mytest$pred=mypred
mytest = mytest[order(mytest$store, mytest$dept, mytest$date), ]
